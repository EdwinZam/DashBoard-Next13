...
export async function generateStaticParams() {
  const data:PokemonsResponse = await fetch(`https://pokeapi.co/api/v2/pokemon?limit=235&offset=151`).
  then(res => res.json());
...
export async function generateMetadata({params}:Props): Promise<Metadata> {
 try {
  ...
  return {
    ...
    description:'Error al cargar la página del Pokémon '
  }
 }
}
            ...
            <div className="flex flex-wrap">
              {
                pokemon.stats.map(stat => (
                  <p key={stat.stat.name} className="mr-2 capitalize font-bold">{stat.stat.name} <span>{stat.base_stat}</span></p>
                ))
              }
            </div> 
          </div>
